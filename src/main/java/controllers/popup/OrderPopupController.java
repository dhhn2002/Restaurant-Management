...
public class OrderPopupController {
    ...
    public void updateAmount(EditOrderPopupView view, Order order) {
        order.setTotalAmount(orderItemController.getTotalAmount());   //Tổng tiền
        view.getLbStatus().setText(order.getStatus().getName());      //Trạng thái
        view.getLbDiscount().setText(order.getDiscount() + "");       //Giảm giá
        view.getLbPaidAmount().setText(formatter.format(order.getPaidAmount()));   //Đã trả
        view.getLbFinalAmount().setText(formatter.format(order.getFinalAmount())); //Tiền phải trả
        view.getLbTotalAmount().setText(formatter.format(order.getTotalAmount())); //Tổng tiền
    }
    ...
    public boolean addOrder(AddOrderPopupView view) throws Exception {
        ...
        e.setOrderDate(new Timestamp(System.currentTimeMillis()));
...
//        }
        ...
        if (order.getFinalAmount() <= 0 || order.getFinalAmount() > order.getPaidAmount()) {// Chưa thanh toán 
            ...
            // Thanh toán
            order.setStatus(OrderStatus.PAID);
//            order.setOrderDate(new Timestamp(System.currentTimeMillis())); //orderdate
    ...
    public void add(AddOrderPopupView view, SuccessCallback sc, ErrorCallback ec) {
        ...
        try {
            for (Table table : tableDao.getAll()) {
                if (table.getStatus() == TableStatus.FREE) {
                    view.getTbComboBoxModel().addElement(table); //Thêm bàn vào cbb
                }
            }
            
            for (OrderType ot : OrderType.values()) {
                view.getCboType().addItem(ot.getName());  //Thêm loại hóa đơn vào cbb
            }
        ...
        view.getBtnOK().addActionListener(evt -> {
            try {
                addOrder(view);
//                editOrder(view,order);  /////EDITTTTTTTTTTTTTTTTTTTT
                view.dispose();
...
//        }
        ...
        view.setVisible(true);
        view.getBtnCancel().addActionListener(evt -> view.dispose()); //Nút cancel
...
//                        "Nhập số tiền thanh toán!", order.getPaidAmount());
//                if (rawInput == null) {
//                    return;
//                }
//                int paidAmount = Integer.parseInt(rawInput);
//                if (order.getFinalAmount() > paidAmount) {
//                    JOptionPane.showMessageDialog(null, "Bạn còn phải thanh toán " 
//                            + formatter.format(order.getFinalAmount() - paidAmount) + " VND");
//                } else {
...
//                order.setPaidAmount(paidAmount);
        ...
        view.getBtnShipManager().addActionListener(evt -> {
            ...
            shipmentPopupController.add(new ShipmentPopupView(), order.getId(),
                    () -> view.showMessage("Tạo / sửa đơn ship thành công!"), view::showError);
        });
