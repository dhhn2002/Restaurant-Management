...
public class CustomerPopupController {
    ...
    public void addCustomer(CustomerPopupView view) throws Exception {
        String name = view.getTxtName().getText(), address = view.getTxtAddress().getText(),
                phoneNumber = view.getTxtPhoneNumber().getText();
        if (phoneNumber.isEmpty() || address.isEmpty() || name.isEmpty()) {
    ...
    public boolean editCustomer(CustomerPopupView view, Customer c) throws Exception {
        ...
        if (phoneNumber.isEmpty()) {
            throw new Exception("Vui lòng điền số điện thoại");
        }
        c.setAddress(address);
        c.setName(name);
        c.setPhoneNumber(phoneNumber);
