...
public class StatisticalEmployeeController {
    ...
    public void initData() { //Ngày bđ, kt
    ...
    public void addEvent() {
        ...
        PropertyChangeListener eventListener = evt -> 
                debouncer.debounce("date_change", onDateChange, 1000);//Chờ 1s không thay đổi mới hiển thị data
    ...
    private void renderEmployee(Timestamp start, Timestamp end) throws SQLException {
        ...
        for (Statistical.SalaryEmployee salaryEmployee : statisticalDao.getSalaryEmployee(start, end)) {
            view.getEmployeeModel().addRow(new Object[]{
                salaryEmployee.employee.getId(), salaryEmployee.employee.getName(),
                salaryEmployee.employee.getSalary(), salaryEmployee.quantily,
                salaryEmployee.bonus, salaryEmployee.getTotal()
            });
        }
    }
