...
//    }
    ...
    public ArrayList<Integer> getDay(int id, int month, int year) throws SQLException {
        String query = "SELECT  DISTINCT DAY(startTime) as day FROM `session` "
                + "WHERE `idEmployee` = ? AND YEAR(startTime) = ? and MONTH(startTime) = ? "
                + "ORDER BY day ASC";
    ...
    public WorkDay getSales(int id, Timestamp date) throws SQLException {
        ...
        String query = "SELECT DATE(orderDate) as day,COUNT(id) AS amount, "
                + "SUM(totalAmount)as total,(COUNT(id)*2000) as bonus FROM `order` "
                + "WHERE idEmployee = ? AND DATE(orderDate) = DATE(?) AND status = ?";
        PreparedStatement statement = conn.prepareStatement(query);
        statement.setInt(1, id);
    ...
    public int getBonus(int id, int month, int year) throws SQLException {
        String query = "SELECT (COUNT(id)*2000) as bonus FROM `order` "
                + "WHERE idEmployee = ? AND YEAR(orderDate) = ? AND MONTH(orderDate)=? "
                + "AND status = ?";
        PreparedStatement statement = conn.prepareStatement(query);
        statement.setInt(1, id);
        statement.setInt(2, year);
